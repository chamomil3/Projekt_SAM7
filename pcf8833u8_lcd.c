...
void LCDClearRegion(int startX, int startY, int width, int height) {
    long i; // licznik p tli
    int endX = startX + width - 1;   // oblicz ko cow  wsp rz dn  X
    int endY = startY + height - 1;  // oblicz ko cow  wsp rz dn  Y
    
    // Ustawienie adresu wiersza (komenda 0x2B)
    WriteSpiCommand(PASET);  // PASET - Ustawienie wiersza
    WriteSpiData(startY);    // Pocz tkowy wiersz
    WriteSpiData(endY);      // Ko cowy wiersz
    
    // Ustawienie adresu kolumny (komenda 0x2A)
    WriteSpiCommand(CASET);  // CASET - Ustawienie kolumn
    WriteSpiData(startX);    // Pocz tkowa kolumna
    WriteSpiData(endX);      // Ko cowa kolumna
    
    // Zapis do pami ci RAM (komenda 0x2C)
    WriteSpiCommand(RAMWR);  // RAMWR - Komenda zapisu do pami ci
    for(i = 0; i < (width * height); i++) {
        // Zapis bia ych pikseli (czyszcz c obszar)
        WriteSpiData((WHITE >> 4) & 0xFF); // G rne 4 bity koloru
        WriteSpiData(((WHITE & 0xF) << 4) | ((WHITE >> 8) & 0xF)); // Dolne 4 bity i 8 bit w
        WriteSpiData(WHITE & 0xFF); // Dolne 8 bit w koloru
    }
}
